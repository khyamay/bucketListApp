angular.module('bucketList.controllers', ['bucketList.services'])
		...
		$scope.createUser = function(){
			...
			API.signup({
				...
				if(error.error && error.error.code == 11000){
					$rootScope.notify('A user with email already exists');
				} else {
					$rootScope.notify("Oops something went wrong, prease ty again");
	...
	.controller('completedCtrl', function ($rootScope, $scope, API, $window){
		$rootScope.$on('fetchCompleted', function (){
			API.getAll($rootScope.getToken()).success(function (data, status, headers, config){
				...
				$rootScope.notify("Opps something went wrong!! Please try again!");
			});
		});
	...
	.controller('newCtrl', function ($rootScope, $scope, API, $window){
		...
		$scope.createNew = function (){
			...
			$rootScope.show("Please wait... Creating new item");
			...
			API.saveItem(form, form.user)
				...
				.error(function (data, status, headers, config){
					$rootScope.hide();
					$rootScope.notify("Opps something went wrong!! Please try it agian");
				});
		};
	});
